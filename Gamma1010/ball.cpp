...
class Gamma1010 :public Rand{
private:
    std::mt19937 rng;  // Mersenne Twister 伪随机数生成器
    std::gamma_distribution<double> dist;

public:
    Gamma1010() : rng(std::chrono::steady_clock::now().time_since_epoch().count()),
                  dist(10.0, 10.0) {}
...
double a[N],e[2][N];
double lp_norm(pair<int,int> v,double p){
    return powl(powl(1.0*v.first,p)+powl(1.0*v.second,p),1.0/p);
}
double lp_norm(double x,double y,double p){
    return powl(powl(1.0*x,p)+powl(1.0*y,p),1.0/p);
}
...
double gain_var(vector<pair<int,int> > ans,double p){
        assert(ans.size()>0);
        double len=lp_norm(ans[0],p),var=0;
        for(int i=0;i<ans.size();++i){
            double x=1.0*ans[i].first/len;
            double y=1.0*ans[i].second/len;
            double len_xy=lp_norm(x,y,p);
            var+=powl(len_xy-1,2);
    	    //fprintf(f_ball,"(%.6lf,%.6lf),",x,y);
        }
    return var;
}
int main(){
    ...
    double p_left=1,p_right=2;
    while(fabs(p_right-p_left)>1e-4){
        double p_1=p_left+(p_right-p_left)/3;
        double p_2=p_left+(p_right-p_left)/3*2;

        if(gain_var(ans,p_1)>gain_var(ans,p_2)) p_left=p_1;
        else p_right=p_2;
    }
    double p=(p_left+p_right)/2;
        assert(ans.size()>0);
        double len=lp_norm(ans[0],p),var=0;
        fprintf(stderr,"{%.5lf}",len/(n-width));
        for(int i=0;i<ans.size();++i){
            double x=1.0*ans[i].first/len;
            double y=1.0*ans[i].second/len;
            double len_xy=lp_norm(x,y,p);
        ...
        fprintf(f_ball,"(%.6lf,%.6lf),",p,gain_var(ans,p));
        fclose(f_ball);
