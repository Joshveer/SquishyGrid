...
int main(){
    double pi=acos(-1);
    FILE* f[]={fopen("output_ball_0d.txt","w"),fopen("output_ball_27d.txt","w"),fopen("output_ball_45d.txt","w")};
    double std_angles[]={0,pi/8,pi/4};
    srand(time(0));
    Plan pl=Plan({0.44273,0.55727},{0.41223,4.73269});
    for(int _width=40;_width<=1000;_width+=40){
        width=_width;
        vector<pair<pair<int,int>,double>> ans=estimate(pl);
        for(int j=0;j<3;++j){
            for(int i=0;i<ans.size();++i){
                double len=ans[i].second/ans[0].second;
                double angle=atan2(1.0*ans[i].first.second-width,1.0*ans[i].first.first-width);
                if(angle<=std_angles[j]){
                    //fprintf(stderr,"{%.5lf}",angle);
                    fprintf(f[j],"(%d,%.6lf),",_width,ans[i].second);
                    break;
                }
            }
        }
