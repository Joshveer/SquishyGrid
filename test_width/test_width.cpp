...
ll n=30000,width=3000; //,ar_sz=arr_size(width);
double _atan;
...
double estimate(Plan& r1){
    r1.init();
    double ans1=attempt(static_cast<Rand*>(&r1),0,r1);
    double ans2=attempt(static_cast<Rand*>(&r1),1,r1);
...
int main() {
    srand(time(0));
    FILE* f0=fopen("output_width_0d.txt","w");
    FILE* f45=fopen("output_width_45d.txt","w");
    Plan pl=Plan({0.23575,0.24575,0.26821,0.25029},{0.0313151204,0.7925472392,14.7165582951,31.0208181649});
    pl.init();
    for(int _width=10;_width<=2500;_width=_width*1.1){
        width=_width;
        double sum_ans0=0,sum_ans45; int num=10;
        for(int i=0;i<num;++i){
            sum_ans0+=attempt(static_cast<Rand*>(&pl),0,pl);
            sum_ans45+=attempt(static_cast<Rand*>(&pl),1,pl);
        }
        sum_ans0/=num;
        sum_ans45/=num;
        fprintf(stderr,"{%d,%.5lf}",_width,sum_ans0);
        fprintf(f0,"(%d,%.5lf),",_width,sum_ans0);
        fprintf(f45,"(%d,%.5lf)",_width,sum_ans45);
    }
    //printf("{%.5lf}",estimate_2({0.23575,0.24575,0.26821,0.25029},{0.03036,0.76835,14.26727,30.07376}));
    //return 0;
